package cn.dsj.lims.pojo;

public class AssessCheckitem {
    private int id;//考核项目编号
    private String task1;//工作任务1
    private String task2;//工作任务2
    private String task3;//工作任务3
    private String task4;//工作任务4
    private String quality1;//工作质量1
    private String quality2;//工作质量2
    private String quality3;//工作质量3
    private String quality4;//工作质量4
    private String skill1;//工作技能1
    private String skill2;//工作技能2
    private String skill3;//工作技能3
    private String skill4;//工作技能4
    private String attitude1;//工作态度与责任感1
    private String attitude2;//工作态度与责任感2
    private String attitude3;//工作态度与责任感3
    private String attitude4;//工作态度与责任感4
    private String harmony1;//协调性1
    private String harmony2;//协调性2
    private String harmony3;//协调性3
    private String harmony4;//协调性4
    private String discipline1;//纪律性1
    private String discipline2;//纪律性2
    private String discipline3;//纪律性3
    private String discipline4;//纪律性4

    public AssessCheckitem() {
        super();
    }


    public AssessCheckitem(int id, String task1, String task2, String task3, String task4, String quality1, String quality2, String quality3, String quality4, String skill1, String skill2, String skill3, String skill4, String attitude1, String attitude2, String attitude3, String attitude4, String harmony1, String harmony2, String harmony3, String harmony4, String discipline1, String discipline2, String discipline3, String discipline4) {
        this.id = id;
        this.task1 = task1;
        this.task2 = task2;
        this.task3 = task3;
        this.task4 = task4;
        this.quality1 = quality1;
        this.quality2 = quality2;
        this.quality3 = quality3;
        this.quality4 = quality4;
        this.skill1 = skill1;
        this.skill2 = skill2;
        this.skill3 = skill3;
        this.skill4 = skill4;
        this.attitude1 = attitude1;
        this.attitude2 = attitude2;
        this.attitude3 = attitude3;
        this.attitude4 = attitude4;
        this.harmony1 = harmony1;
        this.harmony2 = harmony2;
        this.harmony3 = harmony3;
        this.harmony4 = harmony4;
        this.discipline1 = discipline1;
        this.discipline2 = discipline2;
        this.discipline3 = discipline3;
        this.discipline4 = discipline4;
    }

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getTask1() {
        return task1;
    }

    public void setTask1(String task1) {
        this.task1 = task1;
    }

    public String getTask2() {
        return task2;
    }

    public void setTask2(String task2) {
        this.task2 = task2;
    }

    public String getTask3() {
        return task3;
    }

    public void setTask3(String task3) {
        this.task3 = task3;
    }

    public String getTask4() {
        return task4;
    }

    public void setTask4(String task4) {
        this.task4 = task4;
    }

    public String getQuality1() {
        return quality1;
    }

    public void setQuality1(String quality1) {
        this.quality1 = quality1;
    }

    public String getQuality2() {
        return quality2;
    }

    public void setQuality2(String quality2) {
        this.quality2 = quality2;
    }

    public String getQuality3() {
        return quality3;
    }

    public void setQuality3(String quality3) {
        this.quality3 = quality3;
    }

    public String getQuality4() {
        return quality4;
    }

    public void setQuality4(String quality4) {
        this.quality4 = quality4;
    }

    public String getSkill1() {
        return skill1;
    }

    public void setSkill1(String skill1) {
        this.skill1 = skill1;
    }

    public String getSkill2() {
        return skill2;
    }

    public void setSkill2(String skill2) {
        this.skill2 = skill2;
    }

    public String getSkill3() {
        return skill3;
    }

    public void setSkill3(String skill3) {
        this.skill3 = skill3;
    }

    public String getSkill4() {
        return skill4;
    }

    public void setSkill4(String skill4) {
        this.skill4 = skill4;
    }

    public String getAttitude1() {
        return attitude1;
    }

    public void setAttitude1(String attitude1) {
        this.attitude1 = attitude1;
    }

    public String getAttitude2() {
        return attitude2;
    }

    public void setAttitude2(String attitude2) {
        this.attitude2 = attitude2;
    }

    public String getAttitude3() {
        return attitude3;
    }

    public void setAttitude3(String attitude3) {
        this.attitude3 = attitude3;
    }

    public String getAttitude4() {
        return attitude4;
    }

    public void setAttitude4(String attitude4) {
        this.attitude4 = attitude4;
    }

    public String getHarmony1() {
        return harmony1;
    }

    public void setHarmony1(String harmony1) {
        this.harmony1 = harmony1;
    }

    public String getHarmony2() {
        return harmony2;
    }

    public void setHarmony2(String harmony2) {
        this.harmony2 = harmony2;
    }

    public String getHarmony3() {
        return harmony3;
    }

    public void setHarmony3(String harmony3) {
        this.harmony3 = harmony3;
    }

    public String getHarmony4() {
        return harmony4;
    }

    public void setHarmony4(String harmony4) {
        this.harmony4 = harmony4;
    }

    public String getDiscipline1() {
        return discipline1;
    }

    public void setDiscipline1(String discipline1) {
        this.discipline1 = discipline1;
    }

    public String getDiscipline2() {
        return discipline2;
    }

    public void setDiscipline2(String discipline2) {
        this.discipline2 = discipline2;
    }

    public String getDiscipline3() {
        return discipline3;
    }

    public void setDiscipline3(String discipline3) {
        this.discipline3 = discipline3;
    }

    public String getDiscipline4() {
        return discipline4;
    }

    public void setDiscipline4(String discipline4) {
        this.discipline4 = discipline4;
    }
}
